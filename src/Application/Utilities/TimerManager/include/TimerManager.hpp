...
public:
    ...
    /// @param timer Таймер.
    [[nodiscard]] std::chrono::steady_clock::time_point startAndGetStartTime(Timer& timer)
    {
        timer.start = std::chrono::steady_clock::now();
    ...
    /// @param timer Таймер.
    [[nodiscard]] std::chrono::steady_clock::duration stopAndGetElapsedTime(Timer& timer)
    {
        if (timer.isStarted)
        {
            timer.end = std::chrono::steady_clock::now();
    ...
    /// @return Истекший период времени.
    /// @param timer Таймер.
    [[nodiscard]] std::chrono::steady_clock::duration getElapsedTime(const Timer& timer) const
    {
...
#define START_TIMER_FIRST_TIME_AND_GET_START_TIME(timerManager, timer, startTime) \
    if (timerManager.isStopped(timer)) \
    { \
...
#define STOP_TIMER_AND_GET_ELAPSED_TIME(timerManager, timer, duration) duration = timerManager.stopAndGetElapsedTime(timer)

#define GET_ELAPSED_TIME(timerManager, timer, duration) duration = timerManager.getElapsedTime(timer)
